...
class SaneparProvider(BaseProvider):
    name = "sanepar"
    actions = ["fetch-bill"]
    ...
    async def execute(
        ...
        downloads_path = settings.downloads_dir / "sanepar"
        ...
        try:
            agent = Agent(
                task=(
                    f"Go to the Sanepar website (https://www.sanepar.com.br/). "
                    f"Log in with username '{settings.sanepar_username}' "
                    f"and password '{settings.sanepar_password}'. "
                    f"Navigate to the bills/invoices section. "
                    f"Find and download the most recent water bill as PDF. "
